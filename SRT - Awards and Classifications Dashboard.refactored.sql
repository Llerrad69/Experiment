-- Refactored: SRT - Awards and Classifications Dashboard
-- Purpose: demonstrate moving repeated CASE logic into CTEs for reuse
-- Usage: copy the CTE block into the top of other scripts and join to the mapping CTEs
...
base as (
    select
        ...
        mci.mci_ethc,
        stu.stu_dsbc,
        spr.awd_code as spr_awd_code,
        saw.awd_code as saw_awd_code,
        saw.cla2_code,
        crs.crs_udf1,
        sce.sce_moac,
        rou.rou_name,
    ...
    inner join reporting.cam_saw saw on saw.spr_code = sce.sce_scjc and saw.ayr_code = sce.sce_ayrc
    left join reporting.ins_awd awd2 on awd2.awd_code = saw.awd_code
    left join reporting.ins_awd awd_int on awd_int.awd_code = spr.awd_code
    left join reporting.ins_rou rou on sce.sce_rouc = rou.rou_code
    left join reporting.srs_crs crs on sce.sce_crsc = crs.crs_code
    left join reporting.srs_fac fac on fac.fac_code = rou.rou_udf5
    left join reporting.ins_dpt dpt on dpt.dpt_code = rou.rou_udf6
...
final as (
    select
        b.sce_scjc "Record Count",
        -- classification group using classification_cte lookup
        coalesce(classification_cte.cla_label, 'z_No Classification') "Calculated Classification",
        coalesce(award_scqf_cte.scqf_award_level, spr.awd_code) "SCQF Intended",
        coalesce(award_scqf_cte.scqf_award_level, saw.awd_code) "SCQF Award Level",
        coalesce(e.ethnicity_group, 'Not recorded') "Ethnicity Group",
        coalesce(d.disability_flag, 'No') "Disability",
        coalesce(cl.course_level, '(01) Undergraduate') "Course Level",
        coalesce(m.mode_of_attendance, 'z_Not recorded') "Mode of Attendance",
        b.rou_name "Programme Name (Route)",
        b.fac_name "School",
        b.dpt_name "Discipline",
        b.sce_ayrc "Academic Year (Entry)"
    from base b
    left join classification_cte on classification_cte.cla2_code = b.cla2_code
    left join award_scqf_cte on award_scqf_cte.awd_code = b.saw_awd_code
    left join ethnicity_cte e on e.eth_code = coalesce(b.mci_ethc, b.stu_ethc)
    left join disability_cte d on d.dsb_code = b.stu_dsbc
    left join course_level_cte cl on cl.code = b.crs_udf1
    left join mode_attendance_cte m on m.moac = b.sce_moac
)

select * from final;

-- Notes:
-- - This refactor pulls repeated CASE logic into named CTEs that can be copied into other scripts.
-- - For full parity reproduce all mapping lines (e.g., all award codes, all ethnicity codes) in the CTEs or switch to small mapping tables.
-- - To refactor other scripts, copy the CTE block (everything from WITH through the final CTE) into the target script and adapt the base join block as needed.
