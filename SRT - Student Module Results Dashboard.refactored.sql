-- Refactored: SRT - Student Module Results Dashboard
-- Demonstrates use of shared CTE mapping patterns for repeated logic
...
base as (
    ...
    left join sipr.ins_smr smr on smr.spr_code = sce.sce_scjc and smr.ayr_code = sce.sce_ayrc
    left join sipr.ins_mod mods on mods.mod_code = smr.mod_code
    left join sipr.ins_rou rou on sce.sce_rouc = rou.rou_code
    left join sipr.srs_fac fac on fac.fac_code = rou.rou_udf5
    left join sipr.ins_dpt dpt on dpt.dpt_code = rou.rou_udf6
    left join sipr.men_mre mre on mre.mre_code = stu.stu_code and mre.mre_mrcc = 'STU'
    left join sipr.srs_mci mci on mci.mci_mstc = mre.mre_mstc and mre.mre_mrcc = 'STU'
    where smr.smr_agrg is not null
),

final as (
    select
        b.sce_scjc,
        b.smr_ayr as "Module Exam Year",
        b.mod_code as "Module Code",
        b.mod_name as "Module Code and Name",
        coalesce(e.ethnicity_group,'Not recorded') "Ethnicity",
        case when b.stu_dsbc in ('0','N','U','A','99','98','95') or b.stu_dsbc is null then 'No' else 'Yes' end as "Disability",
        coalesce(g.grp,'z_No Grade Group') "Grade Group",
    ...
    left join grade_group_cte g on g.code = b.smr_agrg
    left join mode_attendance_cte m on m.moac = b.sce_moac
)

select * from final;

-- To adapt: expand mappings in CTEs, add missing grade codes, and extend base joins to include additional lookups.
