-- Reusable CTE library for common lookups and groupings
-- This file defines named CTEs that encapsulate repeated CASE logic and mappings
-- Intended to be included at the top of reporting SQL scripts via COPY/PASTE

-- Ethnicity grouping CTE
WITH
...
mode_attendance_cte as (
    ...
    select 'GYRY','Full-time' from dual union all
...
-- Classification mapping placeholder
-- Classification mapping (maps SAW.CLA2_CODE values to human labels)
classification_cte AS (
    select 'AG' cla2_code, 'Aegrotat (Unclassified Honours)' cla_label from dual union all
    select 'CM','Commendation' from dual union all
    select 'FC','First Class Honours' from dual union all
    select 'FCP','First Class (Provisional)' from dual union all
    select 'HN','Honours' from dual union all
    select 'LS','Lower Second Class Honours' from dual union all
    select 'LSP','Lower Second Class Honours (Provisional)' from dual union all
    select 'MARCH','Master of Architecture' from dual union all
    select 'MARCHD','Master of Architecture with Distinction' from dual union all
    select 'MARCHM','Master of Architecture with Merit' from dual union all
    select 'MT','Merit in Teaching' from dual union all
    select 'OD','Ordinary Degree' from dual union all
    select 'PGDS','Distinction' from dual union all
    select 'PGM','Merit' from dual union all
    select 'TC','Third Class Honours' from dual union all
    select 'TCP','Third Class Honours (Provisional)' from dual union all
    select 'UGDS','Ordinary Degree with Distinction' from dual union all
    select 'UGME','Ordinary Degree with Merit' from dual union all
    select 'UNC','Unclassified Honours' from dual union all
    select 'US','Upper Second Class Honours' from dual union all
    select 'USP','Upper Second Class Honours (Provisional)' from dual union all
    select 'UGM','Certificate of Higher Education with Merit' from dual union all
    select null, 'z_No Classification' from dual
),

-- Grade group mapping (maps SMR.SMR_AGRG codes to simplified grade groups)
grade_group_cte AS (
    select 'A1' code, 'A' grp from dual union all
    select 'A2','A' from dual union all
    select 'A3','A' from dual union all
    select 'A4','A' from dual union all
    select 'A5','A' from dual union all
    select 'B1','B' from dual union all
    select 'B2','B' from dual union all
    select 'B3','B' from dual all union all
    select 'C1','C' from dual union all
    select 'C2','C' from dual union all
    select 'C3','C' from dual union all
    select 'D1','D' from dual union all
    ...
    select 'NM','No Mark Awarded' from dual union all
    select 'CA','Absent' from dual union all
    select 'MC','Absent' from dual union all
    select 'DC','Module Discounted' from dual union all
    select 'P','P' from dual union all
    select 'DS','P' from dual union all
    select 'ME','P' from dual union all
    select 'WD','WD' from dual union all
    select null, 'z_No Grade Group' from dual
),

-- Fundability (SCE.EFID) mapping
fundability_cte AS (
    select '1' code, 'Funded by SFC' name from dual union all
    select 'J','Funded by SFC' from dual union all
    select '2','Not funded by SFC' from dual union all
    select '3','Not eligible for funding' from dual union all
    select null, 'Not recorded' from dual
)
